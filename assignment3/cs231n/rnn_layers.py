...
def rnn_step_backward(dnext_h, cache):
    ...
    db = np.sum(dnext_h * dtanh, axis=0) #(H,)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = {}
    for i in range(T):
        h[:, i, :], cache[i] = rnn_step_forward(x[:, i, :], h_interm, Wx, Wh, b)
        h_interm = h[:, i, :]
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    N, T, H = dh.shape
    x, Wx, Wh, prev_h, next_h = cache[T-1]
    D = x.shape[1]
    dx = np.zeros((N, T, D))
    dh0 = np.zeros((N, H))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros((H,))
    dprev_h_interm = np.zeros((N, H))
    #dprev_h_interm = dh[:, T-1, :]
    for i in reversed(range(T)):
        dx_interm, dprev_h_interm, dWx_interm, dWh_interm, db_interm = rnn_step_backward(dh[:, i, :] + dprev_h_interm, cache[i])
        #dx_interm, dprev_h_interm, dWx_interm, dWh_interm, db_interm = rnn_step_backward(dprev_h_interm, cache[i])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cell = np.zeros((N, H))
    for i in range(T):
        hidden, cell, cache[i] = lstm_step_forward(x[:, i, :], hidden, cell, Wx, Wh, b)
        h[:, i, :] = hidden
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
