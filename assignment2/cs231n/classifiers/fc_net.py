...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dtemp2, grads['W2'], grads['b2'] = affine_backward(dloss, cache2)
        grads['W2'] += self.reg * self.params['W2']
        dtemp1, grads['W1'], grads['b1'] = affine_relu_backward(dtemp2, cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        out, cache1, cache2, cache3, cache4, h, bnout = {}, {}, {}, {}, {}, {}, {}
        out[0] = X
        for i in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
                ...
                bnout[i], cache2[i] = batchnorm_forward(h[i], gamma, beta, self.bn_params[i])
                out[i+1], cache3[i] = relu_forward(bnout[i])
                if self.use_dropout:
        ...
        scores, cache3[self.num_layers-1] = affine_forward(out[self.num_layers-1], self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        #gradients
        dout, dbn, dh = {}, {}, {}
        dout[self.num_layers-1], grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dscores, cache3[self.num_layers-1])
        for i in reversed(range(self.num_layers-1)):
            if self.use_batchnorm:
                if self.use_dropout:
                    dout[i+1] = dropout_backward(dout[i+1], cache4[i])
