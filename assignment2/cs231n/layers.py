        ...
        for f in range(F):    #fth filter
            for j in range(H_new):
                for k in range(W_new):
                    out[i, f, j, k] = np.sum(x_padded[i, :, j*s:j*s+HH, k*s:k*s+WW] * w[f]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):    #ith image
        for f in range(F):    #fth filter
            for j in range(H_new):
                for k in range(W_new):
                    window = x_padded[i, :, j*s:j*s+HH, k*s:k*s+WW]
                    db[f] += dout[i, f, j, k]
                    dw[f] += window * dout[i, f, j, k]
                    dx_padded[i, :, j*s:j*s+HH, k*s:k*s+WW] += w[f] * dout[i, f, j, k]
       ...
       for j in range(C):   
           for k in range(H_new):
               for l in range(W_new):
                   window = x[i, j, k*s:k*s+HH, l*s:l*s+WW]
       ...
       for j in range(C):   
           for k in range(H_new):
               for l in range(W_new):
                   window = x[i, j, k*s:k*s+HH, l*s:l*s+WW]
                   m = np.max(window)
                   dx[i, j, k*s:k*s+HH, l*s:l*s+WW] = (window == m) * dout[i, j, k, l]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_temp = np.reshape(x.transpose(0,2,3,1), (N*H*W, C)) #x_temp->N*D where D=C
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_temp = np.reshape(dout.transpose(0,2,3,1), (N*H*W, C)) #x_temp->N*D where D=C
